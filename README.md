...
1. Abans de consumir el servei de producció, cal fer proves a pre-producció. Per fer les proves a pre-producció, cal primer adherir-se al servei i comunicar certificat i IMEIs que es faran servir (s'explica a l'apartat de Requisits). Un cop fetes les proves a pre-producció, es poden fer proves de connectivitat a producció. Per fer aquestes proves de connectivitat a producció, només cal posar un codi d'agent que comenci amb "USU", com per exemple "USU1". Totes les multes amb aquests codis d'agent es consideren proves, i per tant no entren al sistema de Multes de l'ORGT, i per tant consultes posteriors des d'aquest mateix servei Web contestaran que la multa no existeix. Per fer proves a pre-producció, no feu servir un codi d'agent que comenci amb "USU", perquè no donarà d'alta la multa.
...
* Realitzar el procés de sol·licitud d'adhesió al webservice, tal i com es descriu a l'inici d'aquest web. Com a part d'aquest procés, cal facilitar la part pública del certificat amb el qual s'accedirà al web service. El mateix certificat servirà tant per accedir a proves com a producció. Aquest certificat és necessari tant per cridar el web service com per accedir a l'especificació (wsdl, ajuda REST i schema).
